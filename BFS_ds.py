...
class CalBFSMaxMinDS(object):
    ...
    def cal_BFS_max_ds(self):
        ...
        while len(select_pool) > 0:
            ...
            sort_select_pool_dict = sorted(select_pool_dict.items(), key=lambda item: item[1], reverse=True)
            for v in sort_select_pool_dict:
                select_pool.remove(v[0])
                max_permute_prob = max_permute_prob + log(1 / edge_prob_num)
                edge_prob_num = edge_prob_num + v[1] - 2
                no_add_select_pool.append(v[0])
                add_to_select_pool = set(self.tree.neighbors(v[0])).difference(set(no_add_select_pool))
                select_pool.extend(add_to_select_pool)

        return max_permute_prob

    def cal_BFS_min_ds(self):
        no_add_select_pool = copy.copy(self.unift_list)
        no_add_select_pool.append(self.source)
        select_pool = [node for node in self.tree.neighbors(self.source)]
        select_pool = list(set(select_pool).difference(set(self.unift_list)))
        ...
        while len(select_pool) > 0:
            ...
            select_pool_dict = {k: self.nfeature_temp[k]["degree"] for k in select_pool}
            sort_select_pool_dict = sorted(select_pool_dict.items(), key=lambda item: item[1], reverse=False)
            for v in sort_select_pool_dict:
                select_pool.remove(v[0])
...
if __name__ == '__main__':
    ...
    for i in np.arange(0, node_num, 1):
        print("i:", i)
        cal_max_min_ds = CalBFSMaxMinDS(tree_ka, unift_list, i)
        max_permute_prob = cal_max_min_ds.cal_BFS_max_ds()
        min_permute_prob = cal_max_min_ds.cal_BFS_min_ds()
